...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator("text=Upload Successful! Document Added")).to_be_visible(
                timeout=30000
            )
        except AssertionError:
            raise AssertionError(
                "Test plan failed: Drag-and-drop upload UI did not work as expected or progress indicator did not update. Success alert and redirect to case detail with new document entry not verified."
            )
