...
from config import INVALID_TEST_PASSWORD, TEST_USER_EMAIL, UI_BASE_URL
...
async def run_test():
    ...
    try:
        ...
        await page.get_by_test_id("login-password").fill(INVALID_TEST_PASSWORD)
        await page.get_by_test_id("login-submit").click()

        await expect(page.get_by_test_id("login-error")).to_contain_text(
            "Incorrect email or password", timeout=10000
        )
        await expect(page).to_have_url(f"{UI_BASE_URL}/auth/login")
    finally:
        if context:
            await context.close()
        if browser:
