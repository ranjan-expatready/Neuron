...
class PersonService:
    @staticmethod
    def create_person(db: Session, person_data: PersonCreate, org_id: uuid.UUID) -> Person:
        person = Person(
            org_id=org_id,
    ...
    @staticmethod
    def get_person_by_id(db: Session, person_id: uuid.UUID, org_id: uuid.UUID) -> Optional[Person]:
    ...
    @staticmethod
    def get_persons_by_org(db: Session, org_id: uuid.UUID, skip: int = 0, limit: int = 100) -> List[Person]:
        return db.query(Person).filter(
            Person.org_id == org_id,
            Person.deleted_at.is_(None)
    ...
    @staticmethod
    def update_person(db: Session, person_id: uuid.UUID, org_id: uuid.UUID, person_data: PersonUpdate) -> Optional[Person]:
    ...
    @staticmethod
    def delete_person(db: Session, person_id: uuid.UUID, org_id: uuid.UUID) -> bool:
        person = PersonService.get_person_by_id(db, person_id, org_id)
        if not person:
            return False
    ...
    @staticmethod
    def search_persons(db: Session, org_id: uuid.UUID, query: str, skip: int = 0, limit: int = 100) -> List[Person]:
        return db.query(Person).filter(
            Person.org_id == org_id,
            Person.deleted_at.is_(None),
