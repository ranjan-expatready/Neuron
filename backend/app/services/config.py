...
from ..models.config import ConfigCaseType, ConfigForm, ConfigField, ConfigChecklist, ConfigTemplate, ConfigFeatureFlag
from ..schemas.config import ConfigCaseType as ConfigCaseTypeSchema
import uuid
...
class ConfigService:
    ...
    @staticmethod
    def get_forms_by_case_type(db: Session, case_type_code: str, is_active: Optional[bool] = None) -> List[ConfigForm]:
    ...
    @staticmethod
    def get_fields_by_case_type(db: Session, case_type_code: str, form_code: Optional[str] = None, is_active: Optional[bool] = None) -> List[ConfigField]:
        case_type = ConfigService.get_case_type_by_code(db, case_type_code)
        if not case_type:
            return []
    ...
    @staticmethod
    def get_checklist_by_case_type(db: Session, case_type_code: str, is_active: Optional[bool] = None) -> List[ConfigChecklist]:
    ...
    def get_template_by_code(db: Session, template_code: str) -> Optional[ConfigTemplate]:
        return db.query(ConfigTemplate).filter(
            ConfigTemplate.template_code == template_code,
    ...
    def get_templates_by_type(db: Session, template_type: str) -> List[ConfigTemplate]:
        return db.query(ConfigTemplate).filter(
            ConfigTemplate.template_type == template_type,
            ConfigTemplate.is_active == True
        ).all()

    @staticmethod
    def is_feature_enabled(db: Session, flag_key: str, org_id: Optional[uuid.UUID] = None, user_id: Optional[uuid.UUID] = None) -> bool:
