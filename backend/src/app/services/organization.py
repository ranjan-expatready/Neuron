...
class OrganizationService:
    ...
    def create_organization(
        db: Session, org_data: OrganizationCreate, owner_id: uuid.UUID
    ) -> Organization:
        # Create organization
        org = Organization(name=org_data.name, domain=org_data.domain, type=org_data.type)
        db.add(org)
    ...
    def update_organization(
        db: Session, org_id: uuid.UUID, org_data: OrganizationUpdate
    ...
    def get_organization_members(db: Session, org_id: uuid.UUID) -> list[OrganizationMembership]:
        return (
            db.query(OrganizationMembership)
            .filter(
                OrganizationMembership.org_id == org_id, OrganizationMembership.status == "active"
            )
            .all()
        )
