...
class ConfigService:
    ...
    def get_template_by_code(db: Session, template_code: str) -> Optional[ConfigTemplate]:
        return (
            ...
            .filter(
                ConfigTemplate.template_code == template_code, ConfigTemplate.is_active.is_(True)
            )
            .first()
        )
    ...
    def get_templates_by_type(db: Session, template_type: str) -> list[ConfigTemplate]:
        return (
            db.query(ConfigTemplate)
            .filter(
                ConfigTemplate.template_type == template_type, ConfigTemplate.is_active.is_(True)
            )
            .all()
        )
