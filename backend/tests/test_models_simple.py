...
class TestSimpleModels:
    ...
    def test_user_query_operations(self, simple_db_session):
        ...
        users = [
            SimpleUser(email="user1@example.com", encrypted_password="pass1", first_name="User", last_name="One"),
            SimpleUser(email="user2@example.com", encrypted_password="pass2", first_name="User", last_name="Two"),
            SimpleUser(email="user3@example.com", encrypted_password="pass3", first_name="User", last_name="Three")
        ]
        ...
        user_by_email = simple_db_session.query(SimpleUser).filter(SimpleUser.email == "user2@example.com").first()
        assert user_by_email is not None
        assert user_by_email.first_name == "User"
        assert user_by_email.last_name == "Two"
        
        users_by_name = simple_db_session.query(SimpleUser).filter(SimpleUser.first_name == "User").all()
